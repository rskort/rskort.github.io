  ...
  class SurfaceViewer {
    ...
    async load() {
      try {
        ...
        this.status.textContent = `${this.data.element} slab · ${this.data.atoms.length} atoms · ${this.data.sites.length} ideal sites`;
    ...
    render() {
      ...
      if (this.toggle("sites")) {
        this.data.sites.forEach(site => {
          ...
          this.hitTargets.push({x: point.x, y: point.y, radius: 14, text: `${site.marker}. ${site.label}`});
        });
      }
    }
